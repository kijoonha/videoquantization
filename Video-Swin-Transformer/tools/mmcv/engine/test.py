...
def single_gpu_test(model: nn.Module, data_loader: DataLoader) -> list:
    ...
    #evaluation mode
    model.eval()
    #model: <class 'mmcv.parallel.data_parallel.MMDataParallel'>
    results = []
    ...
    for data in data_loader:
        #data: class 'dict'
        #여기서 data의 키값은 imgs와 label
        with torch.no_grad():
            ##fq=vit의 test_quant.py내용 넣음
            ##__call__이 forward를 override, BaseRecognizer->Recognizer3D->func do_test->extract_feature->backbone 수행
