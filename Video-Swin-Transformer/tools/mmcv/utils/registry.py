...
def build_from_cfg_quant(cfg: Dict,cfg_quant:Dict, 
                   registry: 'Registry',
                   default_args: Optional[Dict] = None) -> Any:
    """Build a module from config dict when it is a class configuration, or
    call a function from config dict when it is a function configuration.
    ...
    if default_args is not None:
        for name, value in default_args.items():
            args.setdefault(name, value)
    obj_type = args.pop('type')
    args['backbone']['cfg_quant']=cfg_quant
    print("args: ", args['backbone']['cfg_quant'])
    if isinstance(obj_type, str):
        obj_cls = registry.get(obj_type)
        if obj_cls is None:
            raise KeyError(
...
class Registry:
    ...
    def __init__(self, name, build_func=None, parent=None, scope=None):
        ...
        self.build_func_quant = build_from_cfg_quant
        ...
        if build_func is None:
            if parent is not None:
                ...
                self.build_func_quant = parent.build_func_quant
            else:
    ...
    def build(self, *args, **kwargs): #build는 그냥 인자 여러개 받아서 가능하니까?)
        return self.build_func(*args, **kwargs, registry=self)

    def build_quant(self, *args, **kwargs):
        return self.build_func_quant(*args, **kwargs, registry=self)
