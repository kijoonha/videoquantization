...
def inference_pytorch_quant (args, cfg_quant, cfg, distributed, data_loader, calib_loader, video_list, quant):
    """Get predictions by pytorch models."""
    ...
    if quant:
        ...
        with torch.no_grad():
            for i,calib in enumerate(calib_loader):
                # imgs = calib['imgs']  # 'imgs'는 입력 텐서의 키라고 가정
                # print(f"Input tensor shape at iteration {i}: {imgs.shape}")
                if i == args.calib_iter-1:
                    model.backbone.model_open_last_calibrate()
                model.to(device_num)
                # calib 딕셔너리 안의 텐서들을 device_num으로 이동
                calib = {key: value.to(device_num) if isinstance(value, torch.Tensor) else value for key, value in calib.items()}

                output = model(return_loss=False,**calib)
                if i==args.calib_iter-1:
                    break
...
def main():
    ...
    calib_loader_setting = dict(
            ...
            dist=distributed,
            shuffle=True)
    calib_loader_setting = dict(calib_loader_setting, **cfg.data.get('calib_dataloader',{}))
    ...
    if args.tensorrt:
        ...
        outputs = inference_pytorch_quant(args, cfg_quant, cfg, distributed, data_loader, calib_loader, video_list, args.quant)
