...
class WindowAttention3D_quant(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print("self.qact1: ", self.qact1)
        # if self.qkv.quant==True:
        #     import sys
        #     sys.exit()
...
class QIntLayerNorm(nn.LayerNorm):
    ...
    def forward(self,
                ...
                out_quantizer=None,
                in_scale_expand=1):
        # print('qintlayernorm mode: ', self.mode) #quant
        if self.mode == 'ln':
...
class SwinTransformerBlock3D_quant(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix, last_quantizer):
        B, D, H, W, C = x.shape
        if C==128: #first layer일때, no weight quant
