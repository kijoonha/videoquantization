...
import sys
sys.path.insert(0, '/home/s3/joonhaki/FQ-ViT/models/ptq')
...
class WindowAttention3D_quant(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv = self.qkv(x)        
        # print("qkv shape: ", qkv.shape)                                #@#5957,5958결과 같은지 확인해보기
        qkv = self.qact1(qkv) 
        qkv = qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2,0,3,1,4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # B_, nH, N, C
        # print("weight shape: ", self.qkv.weight.shape)
        q = q * self.scale
        attn = q @ k.transpose(-2, -1)
        #qct_table 추가
...
class QLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        # print("qkv weight size: ", self.weight.shape)
        if self.calibrate:
...
class SwinTransformerBlock3D_quant(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(2,7,7), shift_size=(0,0,0),
        ...
        self.act_quant_layer=cfg.act_quant_layer
    ...
    def forward_part2(self, x):
        # # 그래프 그리는 코드
        # if x.shape[-1]==1024 and self.qact3.calibrate==False:
        #     import numpy as np
        ...
        #     min_values = torch.min(x_reshape, axis=0).values  # B, D', H', W' 차원을 따라 최소값 계산
        #     max_values = torch.max(x_reshape, axis=0).values  # B, D', H', W' 차원을 따라 최대값 계산
            
        #     # 텐서를 CPU로 이동한 후 numpy로 변환
        #     min_values = min_values.cpu().numpy()
        #     max_values = max_values.cpu().numpy()
        #     range = max_values=min_values
        #     C = x.shape[-1]
        #     # 채널 인덱스를 만듭니다.
        #     channels = torch.arange(C).cpu().numpy()
        #     plt.figure(figsize=(10, 6))
        #     plt.plot(channels, min_values, label='Minimum Value', color='red')
        #     plt.plot(channels, max_values, label='Maximum Value', color='purple')
        #     plt.xlabel("Channel's index")
        #     plt.ylabel('Value')
        #     plt.title('Channel-wise Minimum and Maximum Values')
        #     plt.legend()
        #     plt.show()

        #     # input("Press [enter] to continue.")
        #     # 그래프를 파일로 저장
        #     save_dir = '/home/jhki/Video-Swin-Transformer/work_dirs'  # 원하는 경로로 바꾸세요
        #     from datetime import datetime 
        #     import os
        #     if not os.path.exists(save_dir):
        #         os.makedirs(save_dir)  # 디렉토리가 없으면 생성
        #     current_time = datetime.now().strftime("%Y%m%d_%H%M%S")  # '년월일_시분초' 형식

        #     save_path = os.path.join(save_dir, f'output_{current_time}.png')

        #     # 그래프를 지정된 경로에 저장
        #     plt.savefig(save_path)
        #     "finish saving")
        x = self.norm2(x, self.qact2.quantizer, self.qact3.quantizer)
    ...
    def forward(self, x, mask_matrix, last_quantizer):
        ...
        Args:
            ...
            mask_matrix: Attention mask for cyclic shift.
        """
        if self.act_quant_layer[self.i_layer]==True:
            for m in self.modules():
                if type(m) in [QAct] and m.calibrate==False:
                    m.quant=True           
        elif self.act_quant_layer[self.i_layer]==False:
            for m in self.modules():
                if type(m) in [QAct] and m.calibrate==False:
                    m.quant=False    
        shortcut = x
...
class PatchEmbed3D_quant(nn.Module):
    ...
    def __init__(self, patch_size=(2,4,4), in_chans=3, embed_dim=96, norm_layer=None, quant=False, calibrate=False, cfg=None):
        ...
        self.in_chans = in_chans
        self.embed_dim = embed_dim
        self.norm_layer = norm_layer ##여기ㅓ nn.Conv3d도 한번 봐야함)
        # self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        self.proj = QConv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, quant=quant,
                            calibrate=calibrate,
                            bit_type=cfg.BIT_TYPE_W,
                            calibration_mode=cfg.CALIBRATION_MODE_W,
                            observer_str=cfg.OBSERVER_W,
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C D Wh Ww
        # print("after projection tensor size; ", x.shape)
        if self.norm is not None:
            D, Wh, Ww = x.size(2), x.size(3), x.size(4)
            x = x.flatten(2).transpose(1, 2)
...
class SwinTransformer3D_quant(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_size = patch_size
        self.cfg = cfg_quant
        self.input_quant=self.cfg.input_quant
        self.calibrate = calibrate
        if self.input_quant:
    ...
    def model_quant(self):
        for m in self.modules():
            if type(m) in [QLinear, QAct, QIntSoftmax, QConv3d]:
                m.quant = True
    ...
    def act_dequant(self):
        for m in self.modules():
            if type(m) in [QAct]:
                m.quant=False
    
    def print_model(self):
        output_file="/home/s3/joonhaki/Video-Swin-Transformer/modulelist.out"
        with open(output_file, "w") as f:
            for i, m in enumerate(self.modules()):
                if type(m) in [SwinTransformerBlock3D_quant]:
                    f.write("Exiting at SwinTransformerBlock3D_quant\n")
                    break
                elif type(m) in [QLinear, QAct, QIntSoftmax, QConv3d]:
                   f.write(f"quant module m: {m}\n")
                else:
                    f.write(f"normal module: {m}\n")
        # import sys
        # sys.exit()

    def print_modulist(self):
        # 현재 시간을 '년-월-일_시-분-초' 형식으로 포맷
        current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        
        # 파일 이름에 저장 시간을 포함
        output_file = f"/home/s3/joonhaki/Video-Swin-Transformer/modulelist_{current_time}.out"
        with open(output_file, "w") as f:
            for i, m in enumerate(self.modules()):
                f.write(f" module {i} 번째 : {m.__class__.__name__}\n")
                if type(m) in [QLinear, QAct, QIntSoftmax, QConv3d]:
                    f.write(f"quant module and quant: {m.quant}\n")
                        # 
            # for i, m in enumerate(self.children()):  # 최상위 레벨의 모듈 순회
            #     f.write(f"Layer {i}: {m.__class__.__name__}\n")
            #     if type(m) in [QLinear, QAct, QIntSoftmax, QConv3d]:
            #         f.write(f"quant module\n")
            #         f.write(f"quant: {m.quant}\n")
            #     if list(m.children()):
            #         for j, sub_module in enumerate(m.children()):  # 하위 레벨 모듈 순
            #             f.write(f"  sub-module {j}: {sub_module.__class__.__name__}\n")
            #             if type(sub_module) in [QLinear, QAct, QIntSoftmax, QConv3d]:
            #                 f.write(f"quant module\n")
            #                 f.write(f"quant: {sub_module.quant}\n")
            #             for k, last_module in enumerate(sub_module.children()):
            #                     # if sub_module==last_module:
            #                     #     pass
            #                     if last_module.__class__.__name__=="SwinTransformerBlock3D_quant":
            #                         for blk in enumerate(last_module.modules()):
            #                             if blk==last_module:
            #                                 pass
            #                             f.write(f"blk: {blk}\n")
            #                             if type(last_module) in [QLinear, QAct, QIntSoftmax, QConv3d]:
            #                                 f.write(f"quant: {blk.quant}\n")

            #                         break
            #                     f.write(f"last module: {last_module.__class__.__name__}\n")
            #                     if type(last_module) in [QLinear, QAct, QIntSoftmax, QConv3d]:
            #                         f.write(f"quant: {last_module.quant}\n")
    ...
    def forward(self, x):
        ...
        # block1 = layer1.blocks[0]
        if self.qact2.calibrate==False and self.cfg.print_modulist:
            self.print_modulist()
