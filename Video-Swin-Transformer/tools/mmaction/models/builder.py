...
def build_localizer_quant(cfg, cfg_quant):
    """Build localizer."""
    return LOCALIZERS.build_quant(cfg, cfg_quant)
...
def build_model_quant(cfg, cfg_quant=None,train_cfg=None, test_cfg=None):
    """Build model."""
    args = cfg.copy()
    obj_type = args.pop('type')
    if obj_type in LOCALIZERS:
        return build_localizer_quant(cfg, cfg_quant)
    if obj_type in RECOGNIZERS:
        return build_recognizer(cfg, train_cfg, test_cfg)
    if obj_type in DETECTORS:
        if train_cfg is not None or test_cfg is not None:
